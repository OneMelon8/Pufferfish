...
class Trainer(gym.Env, ABC):
    ...
    def step(self, action):
        ...
        for agent_index in range(2):
            ...
            # ================================
                ...
                # TODO: decide whether to use penalty or not
    ...
    def reset(self):
        ...
        if (curr_checkpoint % 5 == 0 and self.last_load != curr_checkpoint) or self.first_reset:
            ...
            self.old_checkpoint = curr_checkpoint - 5
        ...
        if MIX_FLAG:
            self.old_checkpoint = OLD
            print(f"[System] Loading new PVP agent at checkpoint {self.old_checkpoint}...", end=" ")
            self.opponent_policy = load_trained_agent(self.old_checkpoint)
            print("FAILED" if self.opponent_policy is None else "OK")
            self.last_load = curr_checkpoint
...
if __name__ == "__main__":
    # Set to 0 to create a new model
    MIX_FLAG = False
    CP1 = 300
    CP2 = 250
    checkpoint = 341
